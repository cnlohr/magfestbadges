...
#define	PORT		PORTA
#define	DDR		DDRA
#define	POWER_MASK	_BV(POWER_PULLUP)
...
unsigned char micro_remap_mask( unsigned char c )
{
	unsigned char outmask = POWER_MASK;
	if( c & 1 )	outmask |= LED_MASK;
	if( c & 16 )	outmask |= RESET_MASK;
	if( c & 32 )	outmask |= MOSI_MASK;
	if( c & 64 )	outmask |= MISO_MASK;
	if( c & 128 )	outmask |= SCK_MASK;
	return outmask;
}
...
extern	byte_t	usb_setup ( byte_t data[8] )
{
	...
	mask = 1 << bit;
	if (req == USBTINY_SET) {
	  mask = micro_remap_mask( mask );
	  PORT |= mask | POWER_MASK;
	  return 0;
	}
	if (req == USBTINY_CLR) {
	  mask = micro_remap_mask( mask );
	...
	if (req == USBTINY_DDRWRITE) {
	  mask = micro_remap_mask( data[2] );
	  DDR = mask | _BV(POWER_PULLUP);
	}
	// Programming requests
	if	( req == USBTINY_POWERUP )
	{
		sck_period = data[2];
		PORT &= ~LED_MASK; DDR |= LED_MASK;
		mask = POWER_MASK;
		if	( data[4] )
		{
			mask |= RESET_MASK;
		}
//XXX		PORTD &= ~_BV(4); WAS BUFFEN
...
{
//  PORTD |= _BV(4);
//  DDRA = _BV(6) | _BV(5) | _BV(4); // setup USB pullup, LED pin and buffer select pins to output
//  PORTD = _BV(6) | _BV(4); // pull pull-up and buffer disable high

	DDRA = _BV(LED) | _BV(POWER_PULLUP); //LED and USB Pullup 
	PORT = _BV(POWER_PULLUP); //Pullup only.
